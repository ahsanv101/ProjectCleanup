...
def listener(event):
    ...
    try:
        ...
        for i in names:
        	print('input/'+str(i))
        	if os.path.exists('input/'+str(i)):
        		os.remove('input/'+str(i))
        	else:
        		print("o lala la")
        imagesO, nameO = gatherImages('output')
        if len(imagesO) == 0 and len(nameO) == 0:
        	pass
        else:
        	for i in nameO:
        		x,y=mainCalculate('output/'+str(i),1.27,3.20,59)
        		print("width: ",x)
        		print("height: ", y)
